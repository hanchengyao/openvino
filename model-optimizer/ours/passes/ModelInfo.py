"""
Make this pass run after PipelineFinish to calculate macs for the final graph after all transformation
...
def calculate_op_macs(node: Node):
    ...
    if node.type == 'Convolution' or node.type == 'ConvolutionBackpropData':
        ...
        if len(weight_shape) == 4:  # now only do macs calculation for 2-d convolution.         
...
# transformation type: generic middle pass. If enabled is true, this pass will be run.
# this pass also calculate macs_ops/total_ops and data_movement_ops/total_ops
class ModelInfo(BackReplacementPattern):
    enabled = True
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        total_input_output_tensor_size = 0
        ...
        for node in graph.nodes:
            ...
            if node.kind == 'op' and node.type not in not_operator_op_types:
                ...
                # sum up output sizes of all of this op's outputs
                op_out_ports_dict = node.out_ports()
                for _, out_port in op_out_ports_dict.items():
                    output_shape = np.prod(out_port.data.get_shape())
                    total_input_output_tensor_size += output_shape
            ...
            # add input tensor size to total_input_output_tensor_size
            elif node.kind == 'op' and node.type == 'Parameter':
                total_input_output_tensor_size += np.prod(node.shape)
        ...
        graph.graph['activations_size'] = str(round(total_input_output_tensor_size / 10**6, 2)) + 'M'
