...
class ModelOpsAttrsParser(object):
    ...
    def extract_model_ops_and_attrs(self):
        for op_idx in range(self.subgraph.OperatorsLength()):
            ...
            self.op_parsers_map[op_code_str](op)
    ...
    # [Eason] whole function not modified 
    ...
    def pool2d_parser(self, op, pool_type):
        try:
            from tflite.BuiltinOptions import BuiltinOptions
    ...
    def conv_parser(self, op, conv_type):
        ...
        if is_depthwise_conv:
            ...
            # [Eason] add this key-value to set common conv2d's group num to be 1. 
...
def _input_type(model):
    ...
    for subgraph_index in range(subgraph_count):
        ...
        for input_index in range(inputs_count):
            ...
            input_shape = tensor.ShapeAsNumpy()  # [Eason] in NHWC format
            input_shape = tuple(input_shape[np.array([0, 3, 1, 2])])  # [Eason] to NCHW format that is suitable for openvino
