'''
This script determines how to serialize the graph to xml file.
By writting this script, we aim to generate xml in the format and with the attributes we want.
Basically, most of the parts of the xml generating processs here are copied from mo/back/ie_ir_v2/emitter.py .
Based on emitter.py we add new attributes to be shown and remove those we don't care.
The most significant modification we made is in how we serialize op's in/output in xml. This part is 
implemented in xml_layers_connected_by_names function and it is used to replace the 
xml_ports function in emitter.py
'''
...
from ours.utils import np_data_type_to_sting, store_quan_scale_tensor
...
# [Eason] our xml elements
def one_line_shape(shape: np.ndarray, element: Element):
    if len(shape) > 0:
...
def xml_layers_connected_by_names(node: Node, graph, element: Element, edges: Element):
    # input ports
    ...
    for u, d in node.get_sorted_inputs():
        if 'bin' not in d and ('xml_skip' not in d or not d['xml_skip']):
            ...
            if len(in_nodes) == 1:
                if inputs is None:
                    inputs = SubElement(element, 'inputs')
                src, _, out_attrs = in_nodes[0]
                in_node = Node(graph, src)
                in_node_out_port_id = out_attrs['out'] - len(in_node.in_nodes()) if in_node.type != 'Const' else out_attrs['out']
                input = SubElement(inputs, 'input_'+str(d['in']))
                if in_node.type == 'Const' or in_node.type == 'Parameter':
                    input.set('from_input', in_node.name)
                else:
                    input.set('from_layer', str(src))
                    input.set('name', in_node.name)
                    input.set('output_idx', str(in_node_out_port_id))

                # input weight's quantization information
                if in_node.has('weight_scale') and in_node.has('weight_zero_point'):
                    if isinstance(in_node.weight_scale, np.ndarray):  # per-channel quantization's. scale is a tensor of length=output channel
                        scale_file_name = store_quan_scale_tensor(node, d['in'], in_node.weight_scale)
                        input.set('scale', scale_file_name)
                    elif isinstance(in_node.weight_scale, float):
                        input.set('scale', str(in_node.weight_scale))
                    input.set('zero_point', str(in_node.weight_zero_point))

                # input's quantization information
                elif in_node.has('output_scale') and in_node.has('output_zero_point'):
                    if isinstance(in_node.output_scale, np.ndarray):  # per-channel quantization's. scale is a tensor of length=output channel
                        scale_file_name = store_quan_scale_tensor(in_node, d['in'], in_node.output_scale)
                        input.set('scale', scale_file_name)
                    elif isinstance(in_node.output_scale, float):
                        input.set('scale', str(in_node.output_scale))
                    input.set('zero_point', str(in_node.output_zero_point))

                data_type = in_node.out_port(in_node_out_port_id).get_data_type()
                assert data_type is not None
                input.set('dtype', np_data_type_to_sting(data_type))
                one_line_shape(node.graph.node[u]['shape'], input)
    ...
    for v, d in node.get_sorted_outputs():
        if 'xml_skip' not in d or not d['xml_skip']:
            ...
            output = SubElement(outputs, 'output_'+str(port_id))
            ...
            output.set('dtype', np_data_type_to_sting(data_type))
            if node.has('output_scale') and node.has('output_zero_point'):
                if isinstance(node.output_scale, float):  # activation is per-tensor quantized
                    output.set('scale', str(node.output_scale))
                output.set('zero_point', str(node.output_zero_point))
            assert node.graph.node[v]['shape'] is not None, 'Output shape is not calculated properly for node {}' \
                                                            ''.format(node.id)
            one_line_shape(node.graph.node[v]['shape'], output)
# [Eason] our xml elements ends
            ...
            if not isinstance(s, tuple):
                if s == '@ports':
                    try:
                        ...
                        # [Eason] xml elements in our format
                        # xml_ports(node, parent_element, edges)
                        xml_layers_connected_by_names(node, graph, parent_element, edges)
...
def serialize_network(graph, net_element, unsupported):
    ...
    # [Eason] show model info in the begining of the layers part of the xml file
    ...
    for node in nodes:
        ...
        # [Eason] skip emitting nodes that are not opertions in the graph
        # in 
        if node.type in {'Const', 'Parameter', 'Result'} or len(list(graph.predecessors(node.id))) == 0 or len(list(graph.successors(node.id))) == 0:
            continue
...
def generate_v2_ir(graph: Graph, file_name: str, input_names: tuple = (), mean_offset: tuple = (),
