'''
The function prepare_emit_v2_ir in this script do the following:
1. dump numpy for each const and also dump a bin file, then check that we dump numpy files correctly.
2. prepare the graph for our emitter(generate_v2_ir function in ours/emitter_v2.py) to serialize our xml.
3. call generate_v2_ir.

For 2., we relabel nodes in the graph so that the id of the op (not const or param) nodes are set starting
from 0 and in their execution orders.
'''
...
from ours.emitter_v2.emitter_v2 import generate_v2_ir
...
def determined_sort(outputs: list):
    ...
    while len(stack) != 0:
        ...
        if not has_child:
            if node.kind == 'op':
                # [Eason] separate const op nodes and op nodes to different list
                if node.type in {'Const', 'Parameter'}:
                    const_op_order.append(node_id)
                else:
                    op_order.append(node.id)

            if node.kind == 'data':
                data_order.append(node_id)

    return const_op_order, op_order, data_order
...
def prepare_emit_v2_ir(graph: Graph, data_type: str, output_dir: str, output_model_name: str,
    ...
    for sub_graph in [graph] + collect_sub_graphs(graph):
        const_op_order, op_order, data_order = determined_sort(get_sorted_outputs(sub_graph))
        # [Eason] relabel nodes in the way that op nodes' labels would be labeled from 0
        mapping = {v: u for u, v in enumerate(const_op_order, start=-1*len(const_op_order))}
        mapping.update({v: u for u, v in enumerate(op_order)})
    ...
    from ours.utils import dump_numpy_files, check_dumped_params_answer
    dump_numpy_dir = dump_numpy_files(graph)
    check_dumped_params_answer(graph, bin_file, dump_numpy_dir)


    generate_v2_ir(graph=graph,
